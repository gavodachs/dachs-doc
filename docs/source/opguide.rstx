...
published.  If the ``serverURL`` config item on the machine running gavo
pub is pointing to the actual running server, the latter will
...
In defaultmeta.txt, you must give basic information on your authority
and some fallback for services (the defaults all make no sense at all
and should never escape to the VO registry):
...
* managingOrg – an ivo id of the organisation you're running
  the dc for.  The default is ivo://<your authority>/org, the content
...
* title – the name of the organisation on behalf of which
  you are running the data center.  Example: ``Observatory of
  Utopia``
* description and referenceURL – as the
  analoguous item for authority, just for the organisation for which
...
capability to the the resource record of the standard service.  To let
...
* ``css/gavo_dc.css`` – the central CSS; you could use this for 
  ...
  ``@import url("<your css url>");`` to the file the server 
...
There is now a document on `HTML templating in DaCHS`_

.. _HTML templating in DaCHS: http://docs.g-vo.org/DaCHS/templating.html
...
There's a howto over at 
http://docs.g-vo.org/DaCHS/howDoI.html#upgrade-the-database-engine
